...
## Start an AWS Private CA Issuer image

> **Note:** The AWS Private CA Issuer image is primarily designed to run inside a Kubernetes cluster as part of a
> deployment with cert-manager. The standalone Docker command below simply displays configuration options.

Run the following command and replace `<tag>` with the image variant you want to run.

```bash
docker run --rm -it dhi.io/aws-privateca-issuer:<tag> --help
```
...
To install AWS Private CA Issuer using Helm:

```bash
helm install aws-privateca-issuer awspca/aws-privateca-issuer \
    --set "image.repository=dhi.io/aws-privateca-issuer" \
    --set "image.tag=<tag>" \
    --namespace awspca \
    --create-namespace
```
