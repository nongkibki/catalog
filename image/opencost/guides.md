...
To install OpenCost using Helm:

```bash
helm install opencost opencost-charts/opencost \
    --set opencost.exporter.image.registry=dhi.io \
    --set opencost.exporter.image.repository=opencost \
    --set opencost.exporter.image.tag=<tag> \
    --namespace opencost \
    --create-namespace
```
