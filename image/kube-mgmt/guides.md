...
- Public image: `dhi.io/kube-mgmt:<tag>`
- Mirrored image: `<your-namespace>/dhi-kube-mgmt:<tag>`

For the examples, you must first use `docker login dhi.io` to authenticate to the registry to pull the images.

This image runs the `kube-mgmt` binary, a sidecar for managing OPA (Open Policy Agent) instances in Kubernetes. It syncs
Kubernetes resources such as ConfigMaps and Secrets as OPA policies and data. It is designed to be deployed alongside
OPA within a Kubernetes cluster.
...
Use `kube-mgmt` as the entrypoint and `--version` as the command to display version information.

```
$ docker run --rm dhi.io/kube-mgmt:<tag> --version
```

kube-mgmt is typically deployed in Kubernetes as a sidecar container alongside OPA using Helm charts or manifests. For
detailed deployment instructions, configuration options, and integration with OPA, refer to the official documentation
at https://github.com/open-policy-agent/kube-mgmt
