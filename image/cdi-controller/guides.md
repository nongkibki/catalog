...
### What's included in this cdi-controller image

This Docker Hardened CDI Controller image is a component of the
[Containerized Data Importer (CDI)](https://github.com/kubevirt/containerized-data-importer) project from KubeVirt.

- `cdi-controller`: CDI Controller manages the lifecycle of import, upload, and clone operations for virtual machine
  disk images

CDI is designed to work with Kubernetes and KubeVirt. The cdi-controller is one of several CDI components and is
typically deployed by the CDI operator as part of a full CDI installation.

### Run the cdi-controller container

The CDI Controller is designed to run within a Kubernetes cluster as part of the CDI operator deployment. Running it
standalone requires Kubernetes API access and proper configuration.

To display help information:

```bash
docker run --rm dhi.io/cdi-controller:<tag> -h
```

### Deploy CDI in Kubernetes

The recommended way to deploy CDI (including cdi-controller) is using the official CDI operator manifests.

1. Download the CDI operator manifest (replace `<version>` with the CDI release that matches your deployment and DHI
   tag, e.g. `v1.64.0`):
...
3. Create the CDI custom resource to trigger the deployment:
