...
### What's included in this Cilium Operator Azure Hardened Image

This Docker Hardened Cilium Operator Azure image includes the operator component of Cilium. The operator manages
...
### Start a Cilium Operator Azure image

> **Note:** The cilium-operator-azure image is designed to run inside a Kubernetes cluster as part of a complete Cilium
...
### Install Cilium with Cilium Operator Azure using Helm

You can install Cilium with Cilium Operator Azure using the official Helm chart dhiwith your
...
helm upgrade --install cilium cilium/cilium \
  ...
  --set operator.securityContext.enabled=true \
  --set operator.securityContext.runAsNonRoot=true \
  --set operator.securityContext.runAsUser=65532 \
  --set operator.securityContext.runAsGroup=65532
```
...
| Feature         | Non-hardened Cilium Operator Azure | Docker Hardened Cilium Operator Azure               |
| --------------- | ---------------------------------- | --------------------------------------------------- |
...
Switching to the hardened Cilium Operator Azure image does not require any special changes. You can use it as a drop-in
replacement for the standard Cilium Operator Azure (`cilium/cilium-operator-azure`) image in your existing workflows and
