...
To install MySQL Operator using Helm:

```bash
helm install mysql-operator mysql-operator/mysql-operator \
    --set image.repository=dhi.io \
    --set image.name=mysql-operator \
    --set image.tag=<tag> \
    --namespace mysql-operator --create-namespace \
```

Create MySQL InnoDB Cluster installations using defaults or with customization. Specify the mysql-operator image to be
used for the sidecar and init containers via `podSpec`. Here is an example configuration `values.yaml`:
...
podSpec:
  containers:
  - name: sidecar
    image: dhi.io/mysql-operator:<tag>
