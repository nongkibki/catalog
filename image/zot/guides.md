...
- Public image: `dhi.io/zot:<tag>`
- Mirrored image: `<your-namespace>/dhi-zot:<tag>`

For the examples, you must first use `docker login dhi.io` to authenticate to the registry to pull the images.

## Start a Zot registry

### Basic usage

The image includes a default configuration that listens on port 5000 and stores data at `/var/lib/registry`. Start a Zot
registry with:
...
For production use, mount a volume to persist registry data:
...
Mount your own Zot configuration file to `/etc/zot/config.json`:
...
The image ships with a minimal configuration:
...
- `compat: ["docker2s2"]` enables Docker registry v2 compatibility for Docker CLI push/pull.
...
- The hardened image runs as a non-root user (UID 65532). Ensure mounted volumes are accessible to this UID.
- The default configuration has no authentication. Refer to the Zot documentation to configure access control.
- Container listens on `5000/tcp`.
