...
```bash
docker run dhi.io/composer:<tag>-dev --version
```

## Common Composer use cases

### Install a project

Install dependencies for a project using a composer.json without modifying the host.

```bash
docker run --rm -v $(pwd):/app dhi.io/composer:<tag>-dev install
```

### Install a project as part of a multi-stage build

Integrate Composer into your PHP application Dockerfile to build images for deployment.

```dockerfile
FROM dhi.io/composer:<tag> AS builder
COPY . /app
RUN composer install --no-cache --prefer-dist --no-dev --no-autoloader --no-scripts --no-progress

FROM dhi.io/php:<tag>-fpm AS runtime
COPY --from=builder /var/www/html
[...]
```

### Copy Composer into another build stage

```
FROM dhi.io/composer:<tag>-dev AS composer

FROM dhi.io/php:<tag>-dev AS builder
COPY --from=composer /usr/local/bin/composer /usr/local/bin/composer
RUN apt-get install zip # required for Composer
...
RUN composer install --no-cache --prefer-dist --no-dev --no-autoloader --no-scripts --no-progress
```
