...
## Using trust-manager-package with trust-manager via Helm

This image is a companion to [cert-manager trust-manager](https://cert-manager.io/docs/trust/trust-manager/) and is not
intended to be run as a standalone container. Trust-manager uses it as an init container to load a default CA
certificate bundle derived from the Debian `ca-certificates` package.
...
Trust-manager requires cert-manager CRDs to deploy. See https://cert-manager.io/docs/installation/ for details on
...
helm install trust-manager oci://quay.io/jetstack/charts/trust-manager \
  ...
  --set defaultPackageImage.tag="<tag>"
```
