...
### What's included in this virt-handler image

This Docker Hardened Virt Handler image is a component of the [KubeVirt](https://github.com/kubevirt/kubevirt) project.

- `virt-handler`: Virt Handler manages virtual machine processes on each node in a KubeVirt cluster

KubeVirt extends Kubernetes with virtualization capabilities. The virt-handler is one of several KubeVirt components
typically deployed by the KubeVirt operator as part of a full installation.

### Run the virt-handler container

The Virt Handler is designed to run within a Kubernetes cluster as part of the KubeVirt operator deployment.

To display help information:

```bash
docker run --rm dhi.io/virt-handler:<tag> --help
```

Verify the deployed version by inspecting the running KubeVirt pods in your cluster:

```bash
kubectl get pods -n kubevirt -l kubevirt.io=virt-handler -o jsonpath='{.items[*].spec.containers[*].image}'
```

### Deploy KubeVirt in Kubernetes

The recommended way to deploy KubeVirt (including virt-handler) is using the official KubeVirt operator manifests.
Replace `<version>` with the KubeVirt release you intend to run (it should match the virt-handler version you are
standardizing on).
...
**Available image tags for virt-handler:**
...
- Use `dhi.io/virt-handler:<version>` for standard production deployments
