...
- Kubernetes 1.19+
- Kyverno installed in the cluster (Policy Reporter requires Kyverno's PolicyReport CRDs)

To install Kyverno if you haven't already:
...
```console
helm install my-policy-reporter oci://dhi.io/charts/kyverno-policy-reporter-chart --version <version> \
...
```bash
$ kubectl get all -l app.kubernetes.io/name=policy-reporter
NAME                                   READY   STATUS    RESTARTS   AGE
pod/policy-reporter-788cc45d7b-xxxxx   1/1     Running   0          18s

NAME                      TYPE        CLUSTER-IP     EXTERNAL-IP   PORT(S)    AGE
service/policy-reporter   ClusterIP   10.96.48.167   <none>        8080/TCP   18s

NAME                              READY   UP-TO-DATE   AVAILABLE   AGE
deployment.apps/policy-reporter   1/1     1            1           18s
```

You can verify Policy Reporter is working by checking the logs:

```bash
$ kubectl logs -l app.kubernetes.io/name=policy-reporter
... policy report informer sync completed
```
