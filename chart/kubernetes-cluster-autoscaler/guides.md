...
Optionally, you can also use the `--dry-run` flag to test the installation without actually installing anything. To
succesfully deploy the chart, it is required to add AWS configuration.

Create a basic configuration `values.yaml`
...
```console
helm install my-autoscaler oci://dhi.io/kubernetes-cluster-autoscaler-chart --version <version> \
  --set "image.pullSecrets[0]=helm-pull-secret" \
  -f values.yaml
```
...
pod/test-aws-kubernetes-cluster-autoscaler-chart-6b7ffc776f-7n78s   1/1     Running            0             2m10s

NAME                                                   TYPE        CLUSTER-IP    EXTERNAL-IP   PORT(S)    AGE
service/kubernetes                                     ClusterIP   10.43.0.1     <none>        443/TCP    2m22s
service/test-aws-kubernetes-cluster-autoscaler-chart   ClusterIP   10.43.47.69   <none>        8085/TCP   2m10s

NAME                                                           READY   UP-TO-DATE   AVAILABLE   AGE
deployment.apps/test-aws-kubernetes-cluster-autoscaler-chart   1/1     1            1           2m10s

NAME                                                                      DESIRED   CURRENT   READY   AGE
replicaset.apps/test-aws-kubernetes-cluster-autoscaler-chart-6b7ffc776f   1         1         1       2m10s
```
