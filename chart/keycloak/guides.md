...
This chart is configured to run keycloak without the `--optimized` flag, so that custom configuration is possible.

Create a minimal config file to set the hostname which is mandatory for production environments, unless Keycloak is run
with `--hostname-strict=false`.
...
Install Keycloak with this configuration.

```console
helm install my-keycloak oci://dhi.io/keycloak-chart --version <version> \
...
pod/my-keycloak-keycloak-chart-0   1/1     Running   0          104s

NAME                                          TYPE        CLUSTER-IP       EXTERNAL-IP   PORT(S)                    AGE
service/kubernetes                            ClusterIP   10.43.0.1        <none>        443/TCP                    135d
service/my-keycloak-keycloak-chart-headless   ClusterIP   None             <none>        80/TCP                     104s
service/my-keycloak-keycloak-chart-http       ClusterIP   10.43.165.103   <none>        9000/TCP,80/TCP,8443/TCP   104s

NAME                                          READY   AGE
statefulset.apps/my-keycloak-keycloak-chart   1/1     104s
```
